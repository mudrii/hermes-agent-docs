...
RL training tools have been available since v0.2.0. A dedicated RL CLI runner (`rl_cli.py`) provides extended timeouts and RL-focused system prompts.
...
- **WANDB_API_KEY** -- API key for Weights and Biases metrics tracking
...
The agent calls `rl_list_environments()` which scans `tinker-atropos/tinker_atropos/environments/` using AST parsing to find Python classes inheriting from `BaseEnv`. Each environment defines dataset loading, prompt construction, and scoring/verification logic.
...
The agent calls `rl_select_environment("gsm8k_tinker")`, then `rl_get_current_config()`.
...
The agent calls `rl_check_status(run_id)` which reports process status (running/exited for each of the 3 processes), running time, WandB metrics (step, reward mean, percent correct, eval accuracy), and log file locations.

Status checks are rate-limited to once every **30 minutes** per run ID to prevent excessive polling during long-running training jobs.
...
Before committing to a full training run, test an environment with `rl_test_inference`. This runs a few steps of inference and scoring using OpenRouter -- no Tinker API needed, just an `OPENROUTER_API_KEY`.
...
- **3 steps x 16 completions = 48 rollouts per model**
- Tests 3 models at different scales:
  - `qwen/qwen3-8b` (small)
  - `z-ai/glm-4.7-flash` (medium)
  - `minimax/minimax-m2.7` (large)
- Total: ~144 rollouts

This validates that the environment loads correctly, prompt construction works, inference response parsing is robust, and verifier/scoring logic produces valid rewards.
...
4. Takes an optimizer step (Adam: lr=4e-5, beta1=0.9, beta2=0.95)
...
```
Atropos API (run-api, port 8000) <--> Environment (BaseEnv implementation)
                                          |
                                          v
                                     OpenAI / sglang inference API (port 8001)
                                          ^
                                          |
Atropos API --batches: tokens, scores, logprobs--> Tinker Trainer (LoRA training + FastAPI)
                                                          |
                                                          v
                                                   serves inference (port 8001)
```
...
Study the existing `gsm8k_tinker.py` as a template. The agent can help create new environments -- it can read existing environment files, inspect HuggingFace datasets, and write new environment code.

## WandB Metrics

Training runs log to Weights and Biases with these key metrics:
...
The `rl_cli.py` script provides a dedicated CLI mode for RL training workflows with extended timeouts for long-running training and RL-focused system prompts:
...
Requires `TINKER_API_KEY`, `WANDB_API_KEY`, and `OPENROUTER_API_KEY`. The script sets `TERMINAL_CWD` to the `tinker-atropos` submodule directory automatically.
