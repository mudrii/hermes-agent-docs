...
The TUI is the modern front-end for Hermes — a terminal UI backed by the same Python runtime as the [Classic CLI](cli-reference.md). Same agent, same sessions, same slash commands; a cleaner, more responsive surface for interacting with them.
...
The classic CLI remains available as the default. Anything documented in [CLI Interface](cli-reference.md) — slash commands, quick commands, skill preloading, personalities, multi-line input, interrupts — works in the TUI identically.
...
Same [skins](skins.md) and [personalities](personality.md) apply. Switch mid-session with `/skin ares`, `/personality pirate`, and the UI repaints live. See [Skins & Themes](skins.md) for the full list of customizable keys and which ones apply to classic vs TUI — the TUI honors the banner palette, UI colors, prompt glyph/color, session display, completion menu, selection bg, `tool_prefix`, and `help_header`.
...
Keybindings match the [Classic CLI](cli-reference.md) exactly. The only behavioral differences:
...
The per-skin status-bar colors and thresholds are shared with the classic CLI — see [Skins](skins.md) for customization.
...
- [CLI Interface](cli-reference.md) — full slash command and keybinding reference (shared)
- [Sessions](sessions.md) — resume, branch, and history
- [Skins & Themes](skins.md) — theme the banner, status bar, and overlays
- [Voice Mode](voice-mode.md) — works in both interfaces
