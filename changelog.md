...
- `AIAgent.__init__` accepts an optional `stream_callback: callable = None` parameter
- `_run_streaming_chat_completion()` method handles Chat Completions API streaming with graceful fallback to non-streaming on any error
- `_run_codex_stream()` extended to emit text deltas via callback for Codex Responses API
- Thread-safe `queue.Queue()` bridges the agent thread and each consumer (CLI, gateway, API server)
- End-of-stream signaled via `stream_callback(None)`; consumers remove cursors and finalize display
...
Multiple independent tool calls now run in parallel via `ThreadPoolExecutor`, significantly reducing latency for multi-tool turns. Message and result ordering is preserved when reinserting tool responses into conversation history. ([#1152](https://github.com/NousResearch/hermes-agent/pull/1152))
...
Direct Anthropic API calls with Claude Code credential auto-discovery, OAuth PKCE flows, and native prompt caching. No OpenRouter middleman required. ([#1097](https://github.com/NousResearch/hermes-agent/pull/1097))
...
- `hermes claw migrate` command for migrating from OpenClaw — by @kshitijk4poor ([#981](https://github.com/NousResearch/hermes-agent/pull/981), [#1059](https://github.com/NousResearch/hermes-agent/pull/1059))
...
- Strip Hermes provider env vars from all subprocess environments ([#1157](https://github.com/NousResearch/hermes-agent/pull/1157), [#1172](https://github.com/NousResearch/hermes-agent/pull/1172), [#1399](https://github.com/NousResearch/hermes-agent/pull/1399), [#1419](https://github.com/NousResearch/hermes-agent/pull/1419))
...
- Strip Hermes provider/gateway/tool env vars from all subprocess environments ([#1157](https://github.com/NousResearch/hermes-agent/pull/1157)–[#1419](https://github.com/NousResearch/hermes-agent/pull/1419))
...
- Apple/macOS (Notes, Reminders, FindMy, iMessage)
- Autonomous AI Agents (Claude Code, Codex, Hermes spawning, OpenCode)
- Creative (ASCII Art, ASCII Video, Excalidraw)
- MLOps training (Axolotl, TRL, GRPO, Unsloth, PEFT, and 9 more)
- MLOps inference (vLLM, TensorRT-LLM, llama.cpp, and 5 more)
- Vision/multimodal (Whisper, CLIP, SAM, Stable Diffusion, and 2 more)
- Vector databases (Chroma, FAISS, Pinecone, Qdrant)
- Research (ArXiv, domain-intel, blogwatcher, and 3 more)
- GitHub (PR workflow, code review, issues, and 3 more)
- Productivity (Google Workspace, Notion, PDF, PowerPoint)
- Gaming (Minecraft, Pokemon player)
- Media (GIF search, music generation, YouTube)

**New optional skills:** `solana`, `agentmail`, `blackbox`, `neuroskill-bci`, `openclaw-migration`, `qmd`, `1password`, `oss-forensics`.
