...
| `TERMINAL_CWD` | Gateway/cron working directory. Local CLI sessions use the directory where `hermes` was launched. |
...
| `MESSAGING_CWD` | Deprecated gateway working-directory override. Prefer `terminal.cwd` in `config.yaml`. |
...
| `HERMES_REDACT_SECRETS` | `true`/`false` — control secret redaction in logs and shareable outputs (default: `false`). |
