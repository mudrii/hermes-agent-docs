...
| `image_gen` | `image_generate` | Text-to-image generation via pluggable backends — FAL.ai (default), OpenAI `gpt-image-2`, OpenAI Codex OAuth, and xAI `grok-imagine`. Backend is selected via the `plugins/image_gen/{openai,openai-codex,xai}/` plugin layout. |
