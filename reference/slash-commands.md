...
The messaging gateway supports the following built-in commands on platforms that route messages through the generic gateway dispatcher. Availability can vary by platform capabilities, configuration, and plugin support; plugin-registered slash commands are included when the plugin is loaded.
...
- `/status`, `/background`, `/queue`, `/steer`, `/voice`, `/reload-mcp`, `/reload-skills`, `/rollback`, `/debug`, `/fast`, `/footer`, `/curator`, `/kanban`, and `/yolo` work in **both** the CLI and the messaging gateway.
