...
| `reset` | Clear a bundled skill's "user-modified" tracking so `hermes update` resumes overwriting it; pass `--restore` to also re-copy the bundled version. |
...
Print a shell completion script to stdout. Source the output in your shell profile for tab-completion of Hermes commands, subcommands, and profile names. Generated by walking the live argparse parser tree, so completions stay in sync with installed commands.
